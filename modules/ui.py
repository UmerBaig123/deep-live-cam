...
from modules.core import start as start_core
...
def resize_image(image, max_width=1080, max_height=1080):
    h, w = image.shape[:2]
...
def deep_fake_image(source_path: str, target_path: str,output_path: str,enhance=False) -> Image:
    """
    Processes a video frame with optional NSFW filtering and frame processors.
    
    Args:
        source_path (str): Path to the source image.
        target_path (str): Path to the target video.
        frame_number (int, optional): Frame number to process. Defaults to 0.
        nsfw_filter (bool, optional): Whether to apply NSFW filtering. Defaults to False.
        frame_processors (list, optional): List of frame processor modules. Defaults to [].
    ...
    if source_path and target_path:  
        ...
        for frame_processor in get_frame_processors_modules(
            ...
            # temp_frame = output_path 
        image = Image.fromarray(cv2.cvtColor(cv2.imread(output_path), cv2.COLOR_BGR2RGB))
        image = ImageOps.contain(
            image, (PREVIEW_MAX_WIDTH, PREVIEW_MAX_HEIGHT), Image.LANCZOS
        )
        modules.globals.frame_processors=[]
        return image
def deep_fake_live(source: str,target: str,enhance=False) -> Image:
    """
    Processes a video frame with optional NSFW filtering and frame processors.
    ...
    for frame_processor in get_frame_processors_modules(
        ...
        result = frame_processor.process_webcam(
            source,result
        ) 
        torch.cuda.empty_cache()
        # temp_frame = output_path 
...
import numpy as np
def create_video_gradio(output_video_path,fps=30):
    if os.path.exists(output_video_path):  # Check if file exists
        os.remove(output_video_path)  # Remove the file

    ffmpeg_command = [
        "ffmpeg", "-framerate", str(fps), "-i", "temp/input/%04d.png",
        "-c:v", "libx264", "-pix_fmt", "yuv420p","-y", "-vf", "scale=trunc(iw/2)*2:trunc(ih/2)*2",
        output_video_path
    ]
    subprocess.run(ffmpeg_command) 

def deep_fake_video(source_path: str, target_path: str, output_path: str, 
                    enhance=False,nsfw_filter: bool = False, frame_processors: list = []):
    ...
    if modules.globals.keep_fps: 
        ...
        create_video_gradio(modules.globals.output_path, fps)
    else:
        create_video_gradio(modules.globals.output_path)
    # handle audio
    if modules.globals.keep_audio:
        restore_audio(modules.globals.target_path, modules.globals.output_path)
    else:
        move_temp(modules.globals.target_path, modules.globals.output_path)
    temp_path = "temp/input" 
    if os.path.exists(temp_path) and os.path.isdir(temp_path):
        shutil.rmtree(temp_path)  # Remove the directory and its content